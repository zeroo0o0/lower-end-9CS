...
void init_list(list<char> &color_l) {
	//黄、绿、咖啡、蓝、粉红、黑
	for(int i=0;i<str.size();i++) color_l.push_back(str[i]);
}
...
bool check(string s, char goal) {    //比赛记录，目标球
	...
	if(pos!=s.size()) { //打中球但是落空的情况要特判 ***否则可能运行时错误 
		right=s.substr(pos+1); //*直接取到底 
	}
	int num=0;
	...
	if(flag) {
		int tempScore=0;
		for(int j=0; j<s.size(); j++) { //		?红球是算1分还是算num分?  *经实验算一分 
			if(!((s[j]>='0'&&s[j]<='9')||s[j]==' ')) {
...
//		tempScore=max(tempScore,num);
		if(ret==1) {    //记录分数，给对方加分
			scoreB+=max(tempScore,4);
		}
		else if(ret==-1) {
			scoreA+=max(tempScore,4);
		}
		return true;
	}
...
void getScore(string s, char goal) {
	//优先判定不犯规但是没有球落袋的情况
	string temp=s+" ",left="",right=""; //left为击球情况，right为落球情况
	int pos=temp.find(" "); //temp+空格 防止无空格 方便取出left 
	left=temp.substr(0,pos);
	if(pos!=s.size()) { //打中球但是落空的情况要特判
		right=s.substr(pos+1);//pos==s.size() 即落空 
	}
	if(right=="") {
		return; //没有球落袋，不做任何处理
	}
...
AGAIN:
	...
	if(s=="NULL") { //打红球后任意彩球打空，对方加4分
		if(*Ret==1) scoreB+=4;
		else if(*Ret==-1) scoreA+=4;
		change(ret);    //计算单杆得分
		*Ret=-*Ret;
		play(&ret);
		return;
	}
	if(check(s,s[0])) { //把第一个打到的彩球作为目标球 //犯规 
		change(ret);    //计算单杆得分
		*Ret=-*Ret;
		play(&ret);
		return;
